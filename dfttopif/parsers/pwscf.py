        ...
        with open(os.path.join(self._directory, self.outputf)) as fp:
            for line in fp:
                if "PseudoPot. #" in line:
                    ppnames.append(fp.readline().split('/')[-1].rstrip())
        ...
        with open(os.path.join(self._directory, self.outputf), 'r') as fp:
            for line in fp:
                if "crystal axes:" in line:
                    for i in range(3):
                        unit_cell.append([float(j)*alat*bohr_to_angstrom for j in fp.readline().split('(')[-1].split(')')[0].split()])
                    break
                ...
                for line in fp:
                    if "Begin final coordinates" in line:
                        if 'new unit-cell volume' in fp.readline():
                            ...
                            for i in range(3):
                                unit_cell.append([float(j)*cell_conv_factor for j in fp.readline().split()])
                            fp.readline() # blank line

                        # get the final atomic coordinates
                        coordtype = fp.readline().split()[-1].replace('(', '').replace(')', '')
                        if coordtype == 'bohr':
