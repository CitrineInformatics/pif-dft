            ...
            with open(os.path.join(basedir, search_file)) as fp:
                ...
                if return_string:
                    raise Exception('%s not found in %s' %
                                    (' & '.join(search_string), os.path.join(basedir, search_file)))
                else:
                    return False
        else:
            raise Exception('%s file does not exist' % os.path.join(basedir, search_file))
    ...
    def test_if_from(self, directory):
        ...
        for f in files:
            if self._get_line('Program PWSCF', f, basedir=directory, return_string=False):
                ...
                self.inputf = f
            if self.inputf and self.outputf:
                return True
        return False
        ...
        with open(os.path.join(self._directory, self.outputf), 'r') as fp:
            for line in fp:
                if "crystal axes:" in line:
                    for i in range(3):
                        unit_cell.append([float(j)*alat*bohr_to_angstrom for j in
                                          next(fp).split('(')[-1].split(')')[0].split()])
                    break
            if len(unit_cell) == 0:
                raise Exception('Cannot find the initial unit cell')
            ...
            for line in fp:
                if "site n." in line and "atom" in line and "positions" in line and "alat units" in line:
                    for i in range(natoms):
                        ...
                        coords.append([float(j)*coord_conv_factor for j in
                                       coordline.rstrip().split('=')[-1].split('(')[-1].split(')')[0].split()])
                    break
            if len(coords) == 0:
                raise Exception('Cannot find the initial atomic coordinates')

        if self.is_relaxed() is None:
